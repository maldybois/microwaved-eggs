...
class HigherLower(CardGame):
    ...
    async def start_game(self, interaction: discord.Interaction, bet: int):
        ...
        embed = discord.Embed(
            title="High-Low", description="Your card: Higher or Lower?", color=0x005B33
        )
        embed.add_field(
            name="Pot (25% per win): ", value=str(bet) + " gold", inline=False
        )
...
class HigherLowerView(GameView):
    ...
    async def update_game_state(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="Pot (25% per win): ", value=str(int(self.pot)) + " Gold", inline=False
        )
        embed.add_field(
            name="Dealer's card: ", value=self.game.dealer_hand, inline=False
        )
        embed.add_field(name="Your card: ", value="?", inline=False)
        ...
        if self.result_value is not None:
            embed.set_field_at(
                2, name="Your card: ", value=self.game.player_hand, inline=False
            )
            ...
            if self.result_value == 1:
                ...
                embed.set_field_at(
                    0,
                    name="Pot (25% per win): ",
                    value=str(int(self.pot)) + " Gold",
                    inline=False,
                )
                embed.set_field_at(
                    3, name="Current Streak: ", value=self.streak, inline=False
                )
    ...
    def add_higher_lower_buttons(self):
        ...
        higher_button = discord.ui.Button(
            label="Higher", style=discord.ButtonStyle.primary
        )
    ...
    async def handle_continue(self, interaction: discord.Interaction):
        ...
        )  # move current player card to become next round dealers
        ...
        await self.update_game_state(
            interaction
        )  # Update game state to reflect the new round
